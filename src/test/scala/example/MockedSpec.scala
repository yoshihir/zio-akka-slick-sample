...
import example.domain.{ PortfolioAsset, PortfolioAssetRepository, PortfolioId, RepositoryError }
...
class MockedSpec extends AnyFlatSpec with Matchers with Runtime[Unit] with MockitoSugar {
  ...
  "ApplicationService.getPortfolio" should "return a portfolio" in {
    ...
    Mockito
      .when(mockedPortfolioAssetRepository.portfolioAssetRepository.getByPortfolioId(portfolioId))
      .thenReturn(IO {
        List(
    ...
    Mockito
      .when(mockedAssetRepository.assetRepository.getByIds(Set(AssetId(1), AssetId(2))))
      .thenReturn(IO {
        List(Asset(Some(AssetId(1)), "PLN", BigDecimal(1)), Asset(Some(AssetId(2)), "USD", BigDecimal(1)))
      } refineOrDie {
        case e: Exception => RepositoryError(e)
      })
