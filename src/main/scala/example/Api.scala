              ...
              extractHost { host =>
                entity(Directives.as[CreateAssetRequest]) { req =>
                  ApplicationService.addAsset(req.name, req.price).provide(env).map { id =>
                    respondWithHeader(
                      Location(
                        Uri(scheme = scheme).withAuthority(host, port).withPath(Uri.Path(s"/assets/${id.value}"))
                      )
                    ) {
                      complete {
        ...
        path(LongNumber) { assetId =>
          put {
            entity(Directives.as[UpdateAssetRequest]) { req =>
              complete(
                ApplicationService
                  .updateAsset(AssetId(assetId), req.name, req.price)
                  .provide(env)
        ...
        path("assets") {
          put {
            entity(Directives.as[UpdatePortfolioRequest]) { req =>
              complete(
                ApplicationService
                  .updatePortfolio(PortfolioId(portfolioId), AssetId(req.assetId), req.amount)
                  .provide(env)
              )
            }
          }
